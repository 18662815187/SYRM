...
//		HttpSession session = request.getSession();
	...
	public String login(String tel, String password, String verifyCode, Model model, HttpServletRequest request,
...
//						System.out.println("***************这是session的：" + code);
					...
					if (!StringUtils.isEmpty(verifyCode) && verifyCode.equals(code)) {
						...
						if (null != pharmacy) {
							// 4.密码必须正确
							if (pharmacy.getPassword().equals(EncodePassword.encodePassword(password))) {
								...
								session.setAttribute(Constants.USER_NAME, pharmacy.getPharmacyName());
								session.setMaxInactiveInterval(60 * 30);
//								model.addAttribute("pharmacy", pharmacy);
								session.setAttribute("isShow", pharmacy.getIsShow());
								session.setAttribute("pid", pharmacy.getId());
								session.setAttribute("money", pharmacy.getMoney());
//								request.getSession().setAttribute("pid", pharmacy.getId());
								List<Notice> notices = noticeService.findAll();
								session.setAttribute("notices", notices);
								int id = pharmacy.getId();
								LoginRecord loginRecord = loginRecordService.findByPid(id);
								if(loginRecord!=null){
...
//									System.out.println(loginRecord.getLoginTime());
									...
									loginRecord2.setPid(id);
									loginRecord2.setLoginTime(new Date());
									loginRecordService.insertRecord(loginRecord2);
								}else{
